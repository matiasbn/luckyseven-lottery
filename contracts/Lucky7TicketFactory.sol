...
import "./Rhombus/ILighthouse.sol";
import "./Rhombus/Lighthouse.sol";
...
contract Lucky7TicketFactory is Lucky7Admin, usingOraclize {
   ...
   */
  ILighthouse public lucky7Lighthouse;
  ...
  function _askForMuParameter(address _ticketOwner) public oraclizeGasPriceCustomized {
    ...
    if (!isRhombusAvailable) {
      ...
      uint newRandomNumber = Lucky7Library.generateBadRandomNumber(muNonce);
      muNonce++;
      localLighthouse.write(newRandomNumber, 0);
    }
  }
  ...
  function __callback(bytes32 myid, string memory result) public {
    ...
    /** @dev Checks if the result is the i parameter of the user and proceeds the same way, checking if already have the mu parameter setted,
     * and if the user paid for the ticket or the game is in the setting Lucky7Number phase.
     */
    if (iParameterID[myid] != address(0x0) && userValues[iParameterID[myid]].iReady == false) {
